...
def update_task(cluster, service, tag, image, command, env, secret, role, task, region, access_key_id, secret_access_key, profile, diff, exclusive_env, exclusive_secrets):
    """
    ...
    try:
        ...
        click.secho('Update task based on task definition: %s\n' % td.family_revision)
        ...
        create_task_definition(deployment, td)
