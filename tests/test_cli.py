...
def test_update_task_creates_new_revision(get_client, runner):
    ...
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1))
    assert result.exit_code == 0
...
def test_update_task_with_role_arn(get_client, runner):
    get_client.return_value = EcsTestClient('acces_key', 'secret_key')
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1, '-r', 'arn:new:role'))
    assert result.exit_code == 0
    assert not result.exception
    assert u"Update task definition: test-task:1" in result.output
    assert u"Updating task definition" in result.output
...
def test_update_task_new_tag(get_client, runner):
    get_client.return_value = EcsTestClient('acces_key', 'secret_key')
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1, '-t', 'latest'))
    assert result.exit_code == 0
    assert not result.exception
    assert u"Update task definition: test-task:1" in result.output
    assert u"Updating task definition" in result.output
...
def test_update_task_one_new_image(get_client, runner):
    get_client.return_value = EcsTestClient('acces_key', 'secret_key')
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1, '-i', 'application', 'application:latest'))
    assert result.exit_code == 0
    assert not result.exception
    assert u"Update task definition: test-task:1" in result.output
    assert u"Updating task definition" in result.output
...
def test_update_task_one_new_command(get_client, runner):
    get_client.return_value = EcsTestClient('acces_key', 'secret_key')
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1, '-c', 'application', 'foobar'))
    assert result.exit_code == 0
    assert not result.exception
    assert u"Update task definition: test-task:1" in result.output
    assert u"Updating task definition" in result.output
...
def test_update_task_change_environment_variable_empty_string(get_client, runner):
    get_client.return_value = EcsTestClient('acces_key', 'secret_key')
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1, '-e', 'application', 'foo', ''))
...
def test_update_task_new_empty_environment_variable(get_client, runner):
    get_client.return_value = EcsTestClient('acces_key', 'secret_key')
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1, '-e', 'application', 'new', ''))
...
def test_update_task_empty_environment_variable_again(get_client, runner):
    get_client.return_value = EcsTestClient('acces_key', 'secret_key')
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1, '-e', 'webserver', 'empty', ''))
...
def test_update_task_previously_empty_environment_variable_with_value(get_client, runner):
    get_client.return_value = EcsTestClient('acces_key', 'secret_key')
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1, '-e', 'webserver', 'empty', 'not-empty'))
...
def test_update_task_exclusive_environment(get_client, runner):
    get_client.return_value = EcsTestClient('acces_key', 'secret_key')
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1, '-e', 'webserver', 'new-env', 'new-value', '--exclusive-env'))
...
def test_update_task_exclusive_secret(get_client, runner):
    get_client.return_value = EcsTestClient('acces_key', 'secret_key')
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1, '-s', 'webserver', 'new-secret', 'new-place', '--exclusive-secrets'))
...
def test_update_task_without_changing_environment_value(get_client, runner):
    get_client.return_value = EcsTestClient('acces_key', 'secret_key')
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1, '-e', 'webserver', 'foo', 'bar'))
...
def test_update_task_without_changing_secrets_value(get_client, runner):
    get_client.return_value = EcsTestClient('acces_key', 'secret_key')
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1, '-s', 'webserver', 'baz', 'qux'))
...
def test_update_task_without_diff(get_client, runner):
    get_client.return_value = EcsTestClient('acces_key', 'secret_key')
    result = runner.invoke(cli.update_task, (CLUSTER_NAME, TASK_DEFINITION_ARN_1, '-t', 'latest', '-e', 'webserver', 'foo', 'barz', '--no-diff'))
